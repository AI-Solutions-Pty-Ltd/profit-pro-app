...
class Portfolio(BaseModel):
    ...
    def projects_requiring_attention(
        ...
        if not date:
            date = datetime.now()
        attention = []
        urgent_ids = [p.pk for p in self.projects_requiring_urgent_intervention(date)]
        for project in self.active_projects:
    ...
    def cost_variance_at_completion(
        ...
        if not date:
            date = datetime.now()
        eac = self.forecast_cost_at_completion(date)
        if not eac:
            return None
        return self.total_original_budget - eac

    # ==========================================
    # Earned Value Management (EVM)
    # ==========================================

    def total_earned_value(self, date: datetime | None = None) -> Decimal | None:
    ...
    def total_cost_variance(self, date: datetime | None = None) -> Decimal | None:
    ...
    def total_schedule_variance(self, date: datetime | None = None) -> Decimal | None:
    ...
    def total_estimate_at_completion(
        ...
        return self.forecast_cost_at_completion(date)
