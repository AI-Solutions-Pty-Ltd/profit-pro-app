...
class TestProvinceReports:
    ...
    def setup_method(self):
        ...
        self.cert = PaymentCertificateFactory(
            project=self.project, certificate_number=1, status="APPROVED"
        )
    ...
    def test_portfolio_province_summary_view(self, client, monkeypatch):
        ...
        assert "portfolio/reports/province_summary.html" in [
    ...
    def test_province_cover_page_view(self, client, monkeypatch):
        ...
        client.force_login(self.user)
        url = reverse(
            "project:portfolio-province-cover-page",
            kwargs={"province_pk": self.province.pk},
        )
        response = client.get(url)
        assert response.status_code == 200
        assert "portfolio/reports/province_cover_page.html" in [
            t.name for t in response.templates
        ]
    ...
    def test_province_valuation_summary_view(self, client, monkeypatch):
        ...
        client.force_login(self.user)
        url = reverse(
            "project:portfolio-province-valuation-summary",
            kwargs={"province_pk": self.province.pk},
        )
        response = client.get(url)
        assert response.status_code == 200
        assert "portfolio/reports/province_valuation_summary.html" in [
            t.name for t in response.templates
        ]
    ...
    def test_province_cover_page_xlsx_download(self, client, monkeypatch):
        ...
        client.force_login(self.user)
        url = reverse(
            "project:portfolio-province-cover-page-xlsx",
            kwargs={"province_pk": self.province.pk},
        )
        response = client.get(url)
        assert response.status_code == 200
        ...
        assert (
            f"province_{self.province.name.lower()}_cover_page.xlsx"
    ...
    def test_province_valuation_summary_xlsx_download(self, client, monkeypatch):
        ...
        client.force_login(self.user)
        url = reverse(
            "project:portfolio-province-valuation-summary-xlsx",
            kwargs={"province_pk": self.province.pk},
        )
        response = client.get(url)
        assert response.status_code == 200
        ...
        assert (
            f"province_{self.province.name.lower()}_valuation_summary.xlsx"
            in response["Content-Disposition"]
        )
