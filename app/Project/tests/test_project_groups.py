...
class TestProjectGroups:
    ...
    def setup_method(self):
        ...
        self.muni_gp = MunicipalityFactory(
            province=self.province_gp, municipality_name="Johannesburg"
        )
        self.muni_kzn = MunicipalityFactory(
            province=self.province_kzn, municipality_name="Durban"
        )

        self.project1 = ProjectFactory(
            name="GP School", area=self.muni_gp, status=Project.Status.ACTIVE, vat=True
        )
    ...
    def test_group_create_api(self, client):
        ...
        group = ProjectGroup.objects.filter(
            user=self.user, name="My Favorites", deleted=False
        ).first()
    ...
    def test_group_create_duplicate_api(self, client):
        ...
        url = reverse("project:portfolio-group-create")
        post_data = {"name": "My Favorites", "project_ids": f"{self.project1.pk}"}
        response = client.post(
            url, data=json.dumps(post_data), content_type="application/json"
        )
    ...
    def test_group_cover_page_view(self, client, monkeypatch):
        ...
        group = ProjectGroupFactory(
            user=self.user, name="Favorites", projects=[self.project1, self.project2]
        )

        url = reverse("project:portfolio-group-cover-page", args=[group.pk])
        response = client.get(url)
        assert response.status_code == 200
        assert "portfolio/reports/group_cover_page.html" in [
    ...
    def test_group_valuation_summary_view(self, client, monkeypatch):
        ...
        response = client.get(url)
        assert response.status_code == 200
        assert "portfolio/reports/group_valuation_summary.html" in [
            t.name for t in response.templates
        ]
    ...
    def test_group_cover_xlsx(self, client, monkeypatch):
        ...
        client.force_login(self.user)
        group = ProjectGroupFactory(
            user=self.user, name="Favorites", projects=[self.project1]
        )
    ...
    def test_multi_project_valuation_summary_view(self, client, monkeypatch):
        ...
        url = (
            reverse("project:portfolio-multi-valuation-summary")
            + f"?project_ids={self.project1.pk},{self.project2.pk}"
        )
        response = client.get(url)
        assert response.status_code == 200
    ...
    def test_multi_project_valuation_xlsx(self, client, monkeypatch):
        ...
        url = (
            reverse("project:portfolio-multi-valuation-summary-xlsx")
            + f"?project_ids={self.project1.pk},{self.project2.pk}"
        )
        response = client.get(url)
        assert response.status_code == 200
    ...
    def test_group_valuation_pdf(self, client, monkeypatch):
        """Test downloading the group valuation summary PDF."""
        self._apply_mocks(monkeypatch)
        client.force_login(self.user)
        group = ProjectGroupFactory(
            user=self.user, name="Favorites", projects=[self.project1, self.project2]
        ...
        url = reverse("project:portfolio-group-valuation-summary-pdf", args=[group.pk])
        response = client.get(url)
        assert response.status_code == 200
        assert response["Content-Type"] == "application/pdf"
        assert "valuation_summary.pdf" in response["Content-Disposition"]

    def test_multi_project_valuation_pdf(self, client, monkeypatch):
        """Test downloading the ad-hoc multi project valuation summary PDF grouped by province."""
        self._apply_mocks(monkeypatch)
        client.force_login(self.user)

        url = (
            reverse("project:portfolio-multi-valuation-summary-pdf")
            + f"?project_ids={self.project1.pk},{self.project2.pk}"
        )
        response = client.get(url)
        assert response.status_code == 200
