...
class ProjectAssumptionsView(ProjectEstimatorMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return redirect('estimator:project_assumptions', project_pk=self.kwargs['project_pk'])
...
class DashboardView(ProjectEstimatorMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['bill_nos'] = (
            ...
            .values_list('bill_no', flat=True).distinct().order_by('bill_no')
        )
        context['trade_codes'] = ProjectTradeCode.objects.filter(project=project)
        context['spec_names'] = (
            ProjectSpecification.objects.filter(project=project).exclude(name='')
            .values_list('name', flat=True).distinct().order_by('name')
        )
        context['labour_spec_names'] = (
            ProjectLabourSpecification.objects.filter(project=project).exclude(name='')
            .values_list('name', flat=True).distinct().order_by('name')
        )
...
class BaselineBoqView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        ...
        qs = LineItem.objects.filter(
            project=project
        ).select_related('bill', 'bill__structure', 'structure', 'package').order_by('row_index')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['sections'] = (
            ...
            .values_list('bill__name', flat=True).distinct().order_by('bill__name')
        )
...
class SyncBoqView(ProjectEstimatorMixin, View):
    ...
    def post(self, request, project_pk):
        ...
        return redirect(reverse('estimator:dashboard', kwargs={'project_pk': project_pk}))
...
class SpecificationListView(ProjectEstimatorMixin, TemplateView):
    ...
    def _build_calculator_rows(self, project, section_filter=None, trade_code_filter=None, name_filter=None):
        qs = BOQItem.objects.filter(
            project=project,
        ...
        for group in groups.values():
            ...
            for sc in spec.spec_components.select_related('material').all():
                component_totals.append({
                    ...
                    'total_quantity': boq_qty * sc.qty_per_unit if boq_qty else Decimal('0'),
    ...
    def get_context_data(self, **kwargs):
        ...
        context['spec_form'] = context.get('spec_form', SpecificationForm(project=project))
        context['component_formset'] = context.get('component_formset', SpecificationComponentFormSet())

        # Filter options from Output BoQ
        boq_with_spec = BOQItem.objects.filter(
            project=project, specification__isnull=False, is_section_header=False,
        )
        context['sections'] = (
        ...
        context['trade_codes'] = (
            ProjectTradeCode.objects.filter(
                pk__in=boq_with_spec.exclude(specification__trade_code__isnull=True)
                .values_list('specification__trade_code_id', flat=True).distinct()
            )
        )
        context['names'] = (
            boq_with_spec.values_list('specification__name', flat=True).distinct().order_by('specification__name')
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if spec_form.is_valid():
            ...
            spec.save()
            component_formset = SpecificationComponentFormSet(request.POST, instance=spec)
            if component_formset.is_valid():
                component_formset.save()
                return redirect(reverse('estimator:specifications', kwargs={'project_pk': self.kwargs['project_pk']}))
            else:
                spec.delete()
        return self.render_to_response(
            self.get_context_data(spec_form=spec_form, component_formset=component_formset)
        )
...
class MaterialSpecListView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        project = self.get_project()
        qs = ProjectSpecification.objects.filter(
            project=project
        ).select_related(
            'trade_code'
        ).prefetch_related(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if spec_form.is_valid():
            ...
            if component_formset.is_valid():
                component_formset.save()
                return redirect(reverse('estimator:material_specs', kwargs={'project_pk': self.kwargs['project_pk']}))
            else:
                spec.delete()
...
class MaterialsListView(ProjectEstimatorMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            obj.save()
            return redirect(reverse('estimator:materials', kwargs={'project_pk': self.kwargs['project_pk']}))
        self.object_list = self.get_queryset()
...
class LabourCostListView(ProjectEstimatorMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            obj.project = self.get_project()
            obj.save()
            return redirect(reverse('estimator:labour_costs', kwargs={'project_pk': self.kwargs['project_pk']}))
        self.object_list = self.get_queryset()
        return self.render_to_response(self.get_context_data(form=form))
...
class LabourSpecificationListView(ProjectEstimatorMixin, TemplateView):
    ...
    def _build_calculator_rows(self, project, section_filter=None, trade_name_filter=None, name_filter=None):
        qs = BOQItem.objects.filter(
            project=project,
    ...
    def get_context_data(self, **kwargs):
        ...
        boq_with_lspec = BOQItem.objects.filter(
            project=project, labour_specification__isnull=False, is_section_header=False,
        )
        context['sections'] = (
        ...
        context['trade_names'] = (
            ...
            .values_list('labour_specification__trade_name', flat=True).distinct()
            .order_by('labour_specification__trade_name')
        )
        context['names'] = (
            boq_with_lspec.values_list('labour_specification__name', flat=True).distinct()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            obj.project = project
            obj.save()
            return redirect(reverse('estimator:labour_specs', kwargs={'project_pk': self.kwargs['project_pk']}))
        return self.render_to_response(self.get_context_data(form=form))
...
class ExcelImportView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:import_excel', kwargs={'project_pk': self.kwargs['project_pk']})
...
class PricedBoqReportView(ProjectEstimatorMixin, ListView):
    ...
    def _get_amounts(self, item, report_type):
        ...
        baseline_amt = (
            ...
            if item.baseline_new_price and item.contract_quantity else None
        )
...
class MaterialListReportView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        qs = BOQItem.objects.filter(
            ...
            'trade_code', 'specification', 'material',
        ).prefetch_related(
            'specification__spec_components__material',
        ).filter(
            is_section_header=False,
    ...
    def get_context_data(self, **kwargs):
        ...
        if rate_type == 'contract':
            context['parent_template'] = 'estimator/base_baseline_estimator_materials.html'
        else:
        ...
        for row in report_rows:
            if row['amount']:
                ...
                material_totals[m] = material_totals.get(m, Decimal('0')) + row['amount']
        ...
        context['chart_section_values'] = json.dumps([float(s[1]) for s in sorted_sections])

        # Top 10 materials by amount
        sorted_materials = sorted(material_totals.items(), key=lambda x: x[1], reverse=True)[:10]
        context['chart_material_labels'] = json.dumps([m[0] for m in sorted_materials])
        context['chart_material_values'] = json.dumps([float(m[1]) for m in sorted_materials])
...
class LabourListReportView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        qs = BOQItem.objects.filter(
            ...
            'trade_code', 'labour_specification', 'labour_specification__crew',
        ).filter(
            is_section_header=False,
            labour_specification__isnull=False,
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context['chart_comp_skilled'] = json.dumps([crew_composition[c]['skilled'] for c in comp_labels])
        context['chart_comp_semi'] = json.dumps([crew_composition[c]['semi_skilled'] for c in comp_labels])
        context['chart_comp_general'] = json.dumps([crew_composition[c]['general'] for c in comp_labels])
    ...
    def post(self, request, project_pk, pk):
        ...
        if value is None or value == '' or value == 0:
            ...
            try:
                ...
                related_obj = model_cls.objects.get(pk=int(value), project_id=project_pk)
                ...
                try:
                    related_obj = model_cls.objects.filter(
                        name=str(value), section=item.section, project_id=project_pk,
                    ).first()
                    if not related_obj:
                        related_obj = model_cls.objects.filter(
                    ...
                    if related_obj:
                        ...
                        return JsonResponse({'error': f'{field} "{value}" not found'}, status=404)
                except Exception:
                    return JsonResponse({'error': f'{field} "{value}" not found'}, status=404)
            except model_cls.DoesNotExist:
                return JsonResponse({'error': f'{field} with id={value} not found'}, status=404)
    ...
    def _build_response(item):
        ...
        return JsonResponse({
            ...
            'new_materials_amount': fmt(
                ...
                if item.new_materials_rate and item.contract_quantity else None
            ),
            'new_labour_amount': fmt(
                item.new_labour_rate * item.contract_quantity
                if item.new_labour_rate and item.contract_quantity else None
            ),
        })
...
class BulkMarkupUpdateView(View):
    ...
    def post(self, request, project_pk):
        ...
        BOQItem.objects.filter(
            project_id=project_pk, is_section_header=False
        ).update(**{field: decimal_value})
...
class UpdateMaterialView(View):
    ...
    def post(self, request, project_pk, pk):
        ...
        item.save()
        return JsonResponse({'ok': True, 'market_rate': str(round(item.market_rate, 2))})
...
class SystemMaterialSpecListView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        return SystemSpecification.objects.select_related('trade_code').prefetch_related(
            'spec_components__material'
        ).all()
    ...
    def get_context_data(self, **kwargs):
        ...
        context['names'] = SystemSpecification.objects.values_list('name', flat=True).distinct().order_by('name')
        context['f_name'] = self.request.GET.get('name', '')
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if spec_form.is_valid():
            spec = spec_form.save()
            component_formset = SystemSpecificationComponentFormSet(request.POST, instance=spec)
            if component_formset.is_valid():
                component_formset.save()
                return redirect(reverse('estimator:system_specs', kwargs={'project_pk': self.kwargs['project_pk']}))
            else:
                spec.delete()
        self.object_list = self.get_queryset()
        return self.render_to_response(
...
class SystemSpecUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:system_specs', kwargs={'project_pk': self.kwargs['project_pk']})
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.success(self.request, f'System Specs uploaded successfully — {", ".join(msg_parts)}')
        except Exception as e:
...
class DownloadSystemSpecTemplateView(View):
    ...
    def get(self, request, project_pk):
        ...
        if not os.path.exists(template_path):
            ...
            return redirect(reverse('estimator:system_specs', kwargs={'project_pk': project_pk}))

        response = FileResponse(open(template_path, 'rb'), as_attachment=True)
        response['Content-Disposition'] = 'attachment; filename="SystemSpec_Template.xlsx"'
...
class UpdateLabourSpecView(View):
    ...
    def post(self, request, project_pk, pk):
        item = get_object_or_404(ProjectLabourSpecification, pk=pk, project_id=project_pk)
        try:
            data = json.loads(request.body)
        except json.JSONDecodeError:
            return JsonResponse({'error': 'Invalid JSON'}, status=400)
...
class LabourSpecDefListView(ProjectEstimatorMixin, ListView):
    ...
    def get_queryset(self):
        project = self.get_project()
        qs = ProjectLabourSpecification.objects.filter(
            project=project
        ).select_related('crew')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['trade_names'] = (
            ...
            .values_list('trade_name', flat=True).distinct().order_by('trade_name')
        )
        context['names'] = (
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            obj.project = project
            obj.save()
            return redirect(reverse('estimator:labour_spec_defs', kwargs={'project_pk': self.kwargs['project_pk']}))
        self.object_list = self.get_queryset()
        return self.render_to_response(self.get_context_data(form=form))
...
def _handle_upload(request, importer_class, success_url, entity_name, project=None):
    ...
    try:
        ...
        messages.success(request, f'{entity_name} uploaded — {created} created, {updated} updated')
    except Exception as e:
...
class TradeCodeUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:trade_codes', kwargs={'project_pk': self.kwargs['project_pk']})

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['parent_template'] = 'estimator/base_baseline_costs.html'
        ctx['upload_title'] = 'Upload Trade Codes'
        ctx['upload_description'] = 'Upload trade code prefixes and names from an Excel template.'
    ...
    def form_valid(self, form):
        ...
        return _handle_upload(
            self.request, TradeCodeImporter,
            self.get_success_url(), 'Trade Codes',
            project=self.get_project(),
        )
...
class MaterialCostUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:materials', kwargs={'project_pk': self.kwargs['project_pk']})

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['parent_template'] = 'estimator/base_baseline_costs.html'
        ctx['upload_title'] = 'Upload Material Costs'
        ctx['upload_description'] = 'Upload material pricing data from an Excel template.'
    ...
    def form_valid(self, form):
        ...
        return _handle_upload(
            self.request, MaterialCostImporter,
            self.get_success_url(), 'Material Costs',
            project=self.get_project(),
        )
...
class DownloadMaterialCostTemplateView(View):
    def get(self, request, project_pk):
        return _generate_template(
            ['Trade Name', 'Material Code', 'Unit', 'Market Rate', 'Material Variety', 'Market Spec'],
...
class LabourCostUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:labour_costs', kwargs={'project_pk': self.kwargs['project_pk']})
    ...
    def form_valid(self, form):
        ...
        return _handle_upload(
            self.request, LabourCostImporter,
            self.get_success_url(), 'Labour Costs',
            project=self.get_project(),
        )
...
class DownloadLabourCostTemplateView(View):
    def get(self, request, project_pk):
        return _generate_template(
            ['Crew Type', 'Crew Size', 'Skilled', 'Semi Skilled', 'General',
             'Skilled Rate', 'Semi Skilled Rate', 'General Rate'],
...
class MaterialSpecUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:material_specs', kwargs={'project_pk': self.kwargs['project_pk']})

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['parent_template'] = 'estimator/base_materials_estimator.html'
        ctx['upload_title'] = 'Upload Material Specifications'
        ctx['upload_description'] = 'Upload material specification definitions with component breakdowns.'
        ctx['download_url_name'] = 'estimator:download_material_spec_template'
        ctx['columns'] = [
            ('Spec Name', 'Specification name (required) — rows with the same name form one spec'),
    ...
    def form_valid(self, form):
        ...
        return _handle_upload(
            self.request, MaterialSpecImporter,
            self.get_success_url(), 'Material Specifications',
            project=self.get_project(),
        )
...
class DownloadMaterialSpecTemplateView(View):
    def get(self, request, project_pk):
        return _generate_template(
            ['Spec Name', 'Section', 'Trade Code Prefix', 'Unit',
             'Material Code', 'Label', 'Qty per Unit'],
...
class LabourSpecUploadView(ProjectEstimatorMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('estimator:labour_spec_defs', kwargs={'project_pk': self.kwargs['project_pk']})

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['parent_template'] = 'estimator/base_labour_estimator.html'
        ctx['upload_title'] = 'Upload Labour Specifications'
        ctx['upload_description'] = 'Upload labour specification definitions with crew assignments and factors.'
    ...
    def form_valid(self, form):
        ...
        return _handle_upload(
            self.request, LabourSpecImporter,
            self.get_success_url(), 'Labour Specifications',
            project=self.get_project(),
        )
...
class DownloadLabourSpecTemplateView(View):
    def get(self, request, project_pk):
        return _generate_template(
            ['Section', 'Trade Name', 'Name', 'Unit', 'Crew Type',
             'Daily Production', 'Team Mix', 'Site Factor', 'Tools Factor',
             'Leadership Factor'],
...
class SystemTradeCodeUploadView(SystemLibraryMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['upload_description'] = 'Upload trade code prefixes and names from an Excel template.'
    ...
    def form_valid(self, form):
        from .importers import TradeCodeImporter
        return _handle_upload(self.request, TradeCodeImporter, 'estimator:sys_trade_codes', 'Trade Codes')
...
class UpdateSystemMaterialView(View):
    ALLOWED_FIELDS = {'market_rate', 'trade_name', 'material_code', 'unit', 'material_variety', 'market_spec'}
...
class SystemMaterialUploadView(SystemLibraryMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['upload_description'] = 'Upload material pricing data from an Excel template.'
        context['parent_template'] = 'estimator/system/base_system.html'
    ...
    def form_valid(self, form):
        from .importers import MaterialCostImporter
        return _handle_upload(self.request, MaterialCostImporter, 'estimator:sys_materials', 'Materials')
...
class SysMaterialSpecListView(SystemLibraryMixin, ListView):
    ...
    def get_queryset(self):
        qs = super().get_queryset().select_related('trade_code').prefetch_related('spec_components__material')
        section = self.request.GET.get('section')
        if section:
            qs = qs.filter(section=section)
        trade_code = self.request.GET.get('trade_code')
    ...
    def get_context_data(self, **kwargs):
        ...
        context['sections'] = all_specs.exclude(section='').values_list('section', flat=True).distinct().order_by('section')
        context['trade_codes'] = SystemTradeCode.objects.all()
        context['names'] = all_specs.values_list('name', flat=True).distinct().order_by('name')
        context['f_section'] = self.request.GET.get('section', '')
        context['f_trade_code'] = self.request.GET.get('trade_code', '')
        context['f_name'] = self.request.GET.get('name', '')
        return context
...
class SystemMaterialSpecUploadView(SystemLibraryMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['upload_description'] = 'Upload material specification definitions with component breakdowns.'
        context['parent_template'] = 'estimator/system/base_system.html'
        context['download_url_name'] = 'estimator:sys_download_material_spec_template'
        context['columns'] = [
            ('Spec Name', 'Name of the material specification (rows with same name are grouped)'),
            ('Section', 'Section grouping'),
    ...
    def form_valid(self, form):
        from .importers import MaterialSpecImporter
        return _handle_upload(self.request, MaterialSpecImporter, 'estimator:sys_material_specs', 'Material Specs')


class DownloadSystemMaterialSpecTemplateView(SystemLibraryMixin, View):
    def get(self, request):
        return _generate_template(
            ['Spec Name', 'Section', 'Trade Code Prefix', 'Unit', 'Material Code', 'Label', 'Qty per Unit'],
...
class UpdateSystemLabourCrewView(View):
    ALLOWED_FIELDS = {'crew_type', 'crew_size', 'skilled', 'semi_skilled', 'general',
...
class SystemLabourCrewUploadView(SystemLibraryMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['upload_description'] = 'Upload labour crew compositions and daily rates.'
        context['parent_template'] = 'estimator/system/base_system.html'
    ...
    def form_valid(self, form):
        from .importers import LabourCostImporter
        return _handle_upload(self.request, LabourCostImporter, 'estimator:sys_labour_crews', 'Labour Crews')
...
class DownloadSystemLabourCrewTemplateView(SystemLibraryMixin, View):
    def get(self, request):
        return _generate_template(
            ['Crew Type', 'Crew Size', 'Skilled', 'Semi Skilled', 'General',
             'Daily Production', 'Skilled Rate', 'Semi Skilled Rate', 'General Rate'],
...
class SystemLabourSpecListView(SystemLibraryMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['sections'] = all_specs.exclude(section='').values_list('section', flat=True).distinct().order_by('section')
        context['trade_names'] = all_specs.exclude(trade_name='').values_list('trade_name', flat=True).distinct().order_by('trade_name')
        context['names'] = all_specs.values_list('name', flat=True).distinct().order_by('name')
        context['f_section'] = self.request.GET.get('section', '')
        context['f_trade_name'] = self.request.GET.get('trade_name', '')
        context['f_name'] = self.request.GET.get('name', '')
...
class UpdateSystemLabourSpecView(View):
    ALLOWED_FIELDS = {'daily_production', 'team_mix', 'site_factor', 'tools_factor', 'leadership_factor'}
...
class SystemLabourSpecUploadView(SystemLibraryMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['upload_description'] = 'Upload labour specification definitions with crew assignments and factors.'
        context['parent_template'] = 'estimator/system/base_system.html'
    ...
    def form_valid(self, form):
        from .importers import LabourSpecImporter
        return _handle_upload(self.request, LabourSpecImporter, 'estimator:sys_labour_specs', 'Labour Specifications')
...
class DownloadSystemLabourSpecTemplateView(SystemLibraryMixin, View):
    def get(self, request):
        return _generate_template(
            ['Section', 'Trade Name', 'Name', 'Unit', 'Crew Type',
             'Daily Production', 'Team Mix', 'Site Factor', 'Tools Factor', 'Leadership Factor'],
