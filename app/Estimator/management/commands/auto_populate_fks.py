...
class Command(BaseCommand):
    help = (
        "Auto-populate trade_code, specification and labour_specification on BoQ items"
    )
    ...
    def handle(self, *args, **options):
        ...
        if not apply:
            self.stdout.write(
                self.style.WARNING("DRY-RUN mode — use --apply to save\n")
            )
        ...
        stats = self._process_model(
            BOQItem, trade_by_prefix, specs, labour_specs, apply
        )
    ...
    def _process_model(self, model_cls, trade_by_prefix, specs, labour_specs, apply):
        ...
        for item in items:
            ...
            if item.is_section_header:
                ...
                found_labour = self._match_labour_in_text(
                ...
                if desc_upper.startswith("BILL NO.") or desc_upper.startswith(
                    "SECTION NO."
                ):
                    current_spec = None
                    current_labour = None
