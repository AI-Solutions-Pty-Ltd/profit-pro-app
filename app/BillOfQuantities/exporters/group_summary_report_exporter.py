...
def export_group_summary_report_to_xlsx(group_name, province_reports):
    ...
    fill_province_header = PatternFill(start_color="FFEAEAEA", end_color="FFEAEAEA", fill_type="solid")
    fill_subtotal = PatternFill(start_color="FFF5F5F5", end_color="FFF5F5F5", fill_type="solid")
    ...
    title_cell = ws.cell(row=1, column=2, value=f"VALUATION SUMMARY — {group_name.upper()}")
    ...
    # Subtitle Row
    sub_cell = ws.cell(row=2, column=1, value=f"Summary Grouped by Province")
    ...
    for rep in province_reports:
        ...
        prov_header_cell = ws.cell(row=current_row, column=1, value=f"Province: {rep['province_name']}")
        ...
        ws.row_dimensions[current_row].height = 22
        sub_label = ws.cell(row=current_row, column=1, value=f"{rep['province_name']} Sub-total")
