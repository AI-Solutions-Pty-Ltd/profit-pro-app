...
def export_multi_project_cover_page_to_xlsx(projects):
    """
    Export the aggregated cover page report for a custom set of projects to XLSX format.
    Mirrors the styling, colors, and layout of cover_page_exporter.py.
    """
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Aggregated Cover Page"
    ...
    title_cell = ws.cell(row=1, column=3, value="AGGREGATED COVER PAGE")
    ...
    # Row 2: Projects Info
    province_info = f"Selected Active Projects: {project_count}"
    ...
    # Section A info
    info_rows = [
        ("Total Selected Projects", f"{project_count}"),
