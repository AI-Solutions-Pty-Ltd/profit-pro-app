...
for row in range(1, 40):
    for col in range(1, 8):
        ...
        if c.value and type(c.value) == str:
            if 'TOTAL' in c.value.upper() or 'SUMMARY' in c.value.upper() or 'DUE' in c.value.upper():
                ...
                print(f'Row {row} Col {col} - {c.value} - Fill: {fill_color} Font Color: {font_color} Bold: {font_bold}')
